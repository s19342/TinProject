...
function ViewFullDetails() {
    ...
    const view = () => {
        try{
            Axios.get('http://localhost:4000/viewFullDetails').then((response) => {
    ...
    return (
        <div className="viewCustomer">
            <button className="card-button" onClick={toggleView}>Show Full customer product order details</button>
            
            {detailsShown ? (
                        detailsInfo.slice(currentStartIndex, currentEndIndex).map(function(item, i){
                            return <li className="listItemTable" key={i}>Customer Email: {item.email} <p></p> Date of birth: {item.dateofbirth.substring(0,10)} <p></p> Product Name: {item.name} <p></p> Product Description: {item.description} <p></p> Date Purchased: {item.date.substring(0,10)} <p></p> Amount Purchased: {item.quantity}</li>
                        })
            ) : (
            <div></div>
            )}
